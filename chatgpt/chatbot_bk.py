...
class Chatbot:
    ...
    def truncate_conversation(self, convo_id: str = "default") -> None:
        ...
        while True:
            if (
                ...
                while(len(self.conversation[convo_id]) > 2):
    ...
    def reply(self, query, convo_id="default", retry_count=0):
        try:
            ...
            if retry_count < 3:
                time.sleep(30)
                self.logger.warning("[ChatGPT] RateLimit exceed, 第{}次重试".format(retry_count + 1))
            ...
            if retry_count < 1:
                time.sleep(5)
                self.logger.warn("[ChatGPT] Connetion Timeout, 更换proxy, 第{}次重试".format(retry_count + 1))
                for i in range(10):
                    proxy = get_proxy()
